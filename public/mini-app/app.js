...
async function renderSalary(){
  ...
  try{
    ...
    $('salary-form').onsubmit=async e=>{
      ...
      const input={salary:Number(form.elements.salary.value),savingsTarget:Number(form.elements.savingsTarget.value),payday:Number(form.elements.payday.value),currency:form.elements.currency.value,background:[...$('background-rows').children].map(row=>({name:row.querySelector('[name=backgroundName]').value.trim(),amount:Number(row.querySelector('[name=backgroundAmount]').value)}))};
...
function showSalaryResult(result){
  ...
  $('salary-result').innerHTML=`<section class="card hero"><small>${esc(r.cycle.label)}</small><h2>${r.remaining>=0?'Остаток сейчас':'Не хватает уже сейчас'}</h2><div class="total">${esc(fmt(Math.abs(r.remaining)))}</div><p class="note">Зарплата минус все фоновые расходы периода и подтверждённые расходы из приложения. Это расчётный остаток, а не баланс банковского счёта.</p></section><section class="card"><h2>Из чего складывается</h2><div class="salary-line"><span>Зарплата на руки</span><strong>${esc(fmt(r.plan.salary))}</strong></div><div class="salary-line"><span>Фоновые расходы</span><strong>− ${esc(fmt(r.background))}</strong></div><div class="salary-line"><span>В приложении · ${r.count} оп.</span><strong>− ${esc(fmt(r.spent))}</strong></div><p class="muted">Следующая зарплата: ${esc(r.cycle.nextPayday)}. Учитывается только ${esc(r.plan.currency)}, без пересчёта других валют.${r.pendingCount?` Есть неподтверждённые покупки (${r.pendingCount}) — они пока не включены.`:''}</p></section><section class="card"><h2>${r.projectedSavings!==null&&r.projectedSavings<0?'Возможный дефицит к зарплате':'Получится отложить к зарплате'}</h2>${r.projectedSavings===null?'<p class="muted">Прогноз появится после трёх дней периода, когда будет хотя бы один подтверждённый расход.</p>':`<div class="total">≈ ${esc(fmt(Math.abs(r.projectedSavings)))}</div><p class="muted">При сохранении среднего темпа расходов за ${r.cycle.elapsed} дн. Ожидаемые расходы в приложении за весь период: ${esc(fmt(r.projectedExpenses))}. Фоновые расходы вычтены один раз. Неполный день и крупные разовые покупки могут заметно менять прогноз.</p>`}</section>${savingsTargets(r,fmt)}`;
}

function savingsTargets(result,fmt){
  if(!result.targets)return '';
  return `<section class="card"><h2>Сколько тратить, чтобы отложить</h2><p class="muted">Лимит на оставшиеся ${result.remainingDays} дн., включая сегодня, до зарплаты ${esc(result.cycle.nextPayday)}. Уже сделанные покупки и все фоновые расходы вычтены.</p><div class="savings-targets">${[...(result.selectedTarget?[{...result.selectedTarget,selected:true}]:[]),...result.targets.filter(t=>t.target!==result.selectedTarget?.target)].map(t=>`<article class="savings-target"><h3>${t.selected?'Ваша цель: ':'Отложить '}${esc(fmt(t.target))}</h3>${t.achievable?`<strong class="target-daily">${esc(fmt(t.dailyLimit))}<small> в день</small></strong><p>Можно потратить ещё <strong>${esc(fmt(t.remainingBudget))}</strong></p>${t.remainingBudget===0?'<p class="target-warning">Цель достижима только без новых расходов.</p>':''}`:`<strong class="target-warning">Не хватает ${esc(fmt(t.shortfall))}</strong><p>С текущей зарплатой и расходами цель уже недостижима. Нужно увеличить доход или уменьшить фоновые расходы на эту сумму.</p>`}<p class="muted">${t.periodBudget>=0?`Лимит расходов в приложении на весь период: ${esc(fmt(t.periodBudget))}.`:'Зарплаты за вычетом фоновых расходов недостаточно для этой цели даже без новых покупок.'}</p></article>`).join('')}</div><p class="muted">Это максимальные суммы расходов, чтобы сохранить выбранную сумму. Дневной лимит округлён вниз и пересчитывается при открытии калькулятора.</p></section>`;
}
